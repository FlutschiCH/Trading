        ...
        if os.path.exists(self.lock_path):
            ...
            if existing_pid and existing_pid != os.getpid():
                if is_process_running(existing_pid):
                    print(f"{Fore.YELLOW}[LiveWorker Duplicate Check]{Style.RESET_ALL} Worker for Strategy {self.strategy_id} is already actively running in PID {existing_pid} (current PID: {os.getpid()}).\n  -> Lock File: {self.lock_path}", flush=True)
            ...
            if sys.platform == "win32":
                ...
                try:
                    ...
                    if existing_pid and is_process_running(existing_pid):
                        print(f"{Fore.YELLOW}[LiveWorker Duplicate Check]{Style.RESET_ALL} Worker for Strategy {self.strategy_id} is actively running in PID {existing_pid} (current PID: {os.getpid()}).\n  -> Lock File: {self.lock_path}", flush=True)
                        self.lock_file.close()
                        pause_and_exit(0, "Duplicate worker detected. Window will close automatically in 60 seconds (or press Enter)...", timeout=60)
                    elif existing_pid and not is_process_running(existing_pid):
                        print(f"{Fore.YELLOW}[LiveWorker Lock Notice]{Style.RESET_ALL} Previous PID {existing_pid} is dead. Recovering lock for Strategy {self.strategy_id}...\n  -> Lock File: {self.lock_path}", flush=True)
                        self.lock_file.close()
                        try:
                            os.remove(self.lock_path)
                        except Exception:
                        ...
                        print(f"{Fore.YELLOW}[LiveWorker Duplicate Check]{Style.RESET_ALL} Worker for Strategy {self.strategy_id} is locked by another process (current PID: {os.getpid()}).\n  -> Lock File: {self.lock_path}", flush=True)
                        self.lock_file.close()
                        pause_and_exit(0, "Duplicate worker detected. Window will close automatically in 60 seconds (or press Enter)...", timeout=60)
            else:
                import fcntl
                try:
                    ...
                    print(f"{Fore.YELLOW}[LiveWorker Duplicate Check]{Style.RESET_ALL} Worker for Strategy {self.strategy_id} is already running in another process.\n  -> Lock File: {self.lock_path}", flush=True)
                    self.lock_file.close()
                    pause_and_exit(0, "Duplicate worker detected. Window will close automatically in 60 seconds (or press Enter)...", timeout=60)
