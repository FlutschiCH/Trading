...
export const PositionsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const getSelectedAccountId = useCallback(() => {
    ...
    if (savedId && !['none', 'null', 'undefined'].includes(savedId.trim().toLowerCase())) {
      return savedId;
    }
    try {
  ...
  const refreshPositions = useCallback(async (overrideBroker?: string, overrideAccId?: string) => {
    ...
    const candleSource = localStorage.getItem('wyckoff_candle_source') || 'metatrader';
    const broker = overrideBroker || (candleSource === 'localctrader' ? 'ctrader' : candleSource);
