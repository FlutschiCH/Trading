...
export const SymbolMappingCard: React.FC<SymbolMappingCardProps> = ({ isReadOnly = false }) => {
  ...
  // Only list master symbols that the user has actually created in mappings
  const existingMasterSymbols = Array.from(new Set(symbolMappings.map(m => (m.main_symbol || '').toUpperCase().trim()).filter(Boolean))).sort();
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', height: '100%', width: '100%' }}>
      {/* Header Refresh Bar */}
      <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', gap: '6px', backgroundColor: '#0f172a', padding: '6px 10px', borderRadius: '4px', border: '1px solid #1e293b' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
          <span style={{ fontSize: '11px', fontWeight: 'bold', color: '#94a3b8' }}>Symbol Mapping Config</span>
          <DebugComponentBadge name="SymbolMappingCard" />
        </div>
        <button
          type="button"
          onClick={() => {
