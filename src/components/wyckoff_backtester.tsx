  ...
  React.useEffect(() => {
    ...
    if (Array.isArray(backtestResults.equityCurve) && backtestResults.equityCurve.length > 0) {
      ...
      trades.forEach((trade: any) => {
        const pnl = trade.pnl ?? trade.net_pnl ?? trade.profit ?? 0;
        currentBal += pnl;
        if (currentBal > peak) peak = currentBal;
        const dd = peak > 0 ? ((peak - currentBal) / peak) * 100 : 0;
        const time = trade.exitTimestamp || trade.entryTimestamp || trade.timestamp || (points[points.length - 1].time + 3600);
        points.push({ time, value: currentBal, drawdown_pct: dd });
      });
    }
...
interface WyckoffBacktesterProps {
  ...
  backtestRunInfo?: { current: number; total: number; etaSeconds?: number } | null;
...
export default function WyckoffBacktester({
  ...
  const effectiveSymbols = globalRangeMode ? (activeSymbols.length > 0 ? activeSymbols : [symbol]) : [symbol];
  const effectiveTimeframes = globalRangeMode ? (activeTimeframes.length > 0 ? activeTimeframes : [timeframe]) : [timeframe];
  ...
  const handleLoadSavedBacktest = async (id: string) => {
    try {
      ...
      if (json.status === 'success' && json.data) {
        ...
        console.log(`[SavedRun Load] Clicked Load for Run ID "${id}". Raw DB Payload:`, fullPayload);
        console.log(`[SavedRun Load] Strategy Settings Object to load:`, s);

        const replacements: Record<string, any> = {};

        // 1. Basic SL/RR & Sizing (Check settings object first, fallback to top-level MySQL columns)
        const slValRaw = s.slVal ?? s.sl_val ?? fullPayload.sl_val;
        if (slValRaw !== undefined && slValRaw !== null) {
          const val = String(slValRaw);
          setBacktestSL(val);
          replacements.backtestSL = val;
        }

        const slTypeRaw = s.slType ?? s.sl_type ?? fullPayload.sl_type;
        if (slTypeRaw !== undefined && slTypeRaw !== null) {
          const val = String(slTypeRaw) as 'price' | 'pct' | 'dollar';
          setBacktestSLType(val);
          replacements.backtestSLType = val;
        }

        const rrRaw = s.rr ?? fullPayload.rr;
        if (rrRaw !== undefined && rrRaw !== null) {
          const val = String(rrRaw);
          setBacktestRR(val);
          replacements.backtestRR = val;
        }

        const beTriggerRRaw = s.beTriggerR ?? s.be_trigger_r ?? fullPayload.be_trigger_r;
        if (beTriggerRRaw !== undefined && beTriggerRRaw !== null) {
          const val = String(beTriggerRRaw);
          setBacktestBE(val);
          replacements.backtestBE = val;
        }

        if (s.useBreakEven !== undefined || s.use_break_even !== undefined) {
          const val = Boolean(s.useBreakEven ?? s.use_break_even);
          setUseBreakEven(val);
          replacements.useBreakEven = val;
        } else if (beTriggerRRaw !== undefined && beTriggerRRaw !== null) {
          const val = Number(beTriggerRRaw) > 0;
          setUseBreakEven(val);
          replacements.useBreakEven = val;
        }

        if (s.size !== undefined) {
          const val = String(s.size);
          setBacktestSize(val);
          replacements.backtestSize = val;
        }

        if (s.initialBalance !== undefined || s.initial_balance !== undefined) {
          const val = String(s.initialBalance ?? s.initial_balance);
          setBacktestBalance(val);
          replacements.backtestBalance = val;
        }

        if (s.useRiskSizing !== undefined || s.use_risk_sizing !== undefined) {
          const val = Boolean(s.useRiskSizing ?? s.use_risk_sizing);
          setUseRiskSizing(val);
          replacements.useRiskSizing = val;
        }

        if (s.riskPct !== undefined || s.risk_pct !== undefined) {
          const val = String(s.riskPct ?? s.risk_pct);
          setBacktestRiskPct(val);
          replacements.backtestRiskPct = val;
        }

        // 2. Strategy & Rule Controls
        if (s.lookbackWindow !== undefined) {
          const val = String(s.lookbackWindow);
          setLookbackWindow(val);
          replacements.lookbackWindow = val;
        }
        if (s.feesPercent !== undefined || s.fees_percent !== undefined) {
          const val = String(s.feesPercent ?? s.fees_percent);
          setBacktestFees(val);
          replacements.backtestFees = val;
        }
        if (s.dailyRetryLimit !== undefined) {
          const val = String(s.dailyRetryLimit);
          setDailyRetryLimit(val);
          replacements.dailyRetryLimit = val;
        }
        if (s.allowOppositeClose !== undefined) {
          const val = Boolean(s.allowOppositeClose);
          setAllowOppositeClose(val);
          replacements.allowOppositeClose = val;
        }
        if (s.entryStabilityRule !== undefined) {
        ...
        if (s.enabledIndicators !== undefined) {
          setEnabledIndicators(s.enabledIndicators);
          replacements.enabledIndicators = s.enabledIndicators;
        }
        if (s.hiddenStages !== undefined && setHiddenStages) {
        ...
        if (s.indicatorRules && Array.isArray(s.indicatorRules)) {
          setIndicatorRules(s.indicatorRules);
          replacements.indicatorRules = s.indicatorRules;
        } else if (s.indicator_rules && Array.isArray(s.indicator_rules)) {
          setIndicatorRules(s.indicator_rules);
        ...
        if (s.rrEnd !== undefined) {
          setRREnd(s.rrEnd);
          replacements.rrEnd = s.rrEnd;
        }
        if (s.rrStep !== undefined) {
          setRRStep(s.rrStep);
        ...
        if (s.slStart !== undefined) {
          ...
          setSLStart(val);
          replacements.slStart = val;
        }
        if (s.slEnd !== undefined) {
          const val = String(s.slEnd);
          setSLEnd(val);
          replacements.slEnd = val;
        }
        if (s.slStep !== undefined) {
          const val = String(s.slStep);
          setSLStep(val);
        ...
        if (s.beStart !== undefined) {
          ...
          setBEStart(val);
          replacements.beStart = val;
        }
        if (s.beEnd !== undefined) {
          const val = String(s.beEnd);
          setBEEnd(val);
          replacements.beEnd = val;
        }
        if (s.beStep !== undefined) {
          const val = String(s.beStep);
          setBEStep(val);
          replacements.beStep = val;
        }

        // 6. Symbol and Timeframe updates
        const loadedSymbol = s.symbol || fullPayload.symbol;
        const loadedTimeframe = s.timeframe || fullPayload.timeframe;
        if (loadedSymbol && onSymbolChange) {
          onSymbolChange(loadedSymbol);
          replacements.symbol = loadedSymbol;
        }
        if (loadedTimeframe && onTimeframeChange) {
          onTimeframeChange(loadedTimeframe);
          replacements.timeframe = loadedTimeframe;
        }
        ...
        if (onLoadSavedPayload) {
          ...
          onLoadSpecificResults(
            ...
            String(s.slVal ?? s.sl_val ?? '1.0'),
            String(s.rr ?? '2.0'),
            (s.useBreakEven ?? s.use_break_even) ? String(s.beTriggerR ?? s.be_trigger_r ?? '1.0') : 'off'
          );
        }
