            ...
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid #1e293b', paddingBottom: '6px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <span style={{
                    ...
                    fontWeight: 'bold',
                    fontSize: '11px',
                    backgroundColor: (selectedTrade.type || selectedTrade.side || 'BUY').toUpperCase() === 'BUY' ? 'rgba(16, 185, 129, 0.2)' : 'rgba(239, 68, 68, 0.2)',
                    color: (selectedTrade.type || selectedTrade.side || 'BUY').toUpperCase() === 'BUY' ? '#10b981' : '#ef4444',
                    border: `1px solid ${(selectedTrade.type || selectedTrade.side || 'BUY').toUpperCase() === 'BUY' ? '#10b981' : '#ef4444'}`
                  }}>
                    {(selectedTrade.type || selectedTrade.side || 'BUY').toUpperCase()} TRADE #{selectedTrade.id || ''}
                  </span>
                </div>
                <button
                  onClick={() => onSelectTrade ? onSelectTrade(null) : null}
                  style={{ background: 'none', border: 'none', color: '#64748b', cursor: 'pointer', padding: '2px' }}
                  title="Deselect Trade"
                >
                  <X size={16} />
                </button>
              </div>

              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '6px', fontSize: '11px', backgroundColor: '#0f172a', padding: '8px', borderRadius: '6px' }}>
                <div>
                  <div style={{ color: '#64748b', fontSize: '10px' }}>ENTRY</div>
                  <div style={{ fontWeight: 'bold', color: '#f8fafc' }}>{selectedTrade.entryPrice ? Number(selectedTrade.entryPrice).toFixed(5) : 'N/A'}</div>
                </div>
                <div>
                  <div style={{ color: '#64748b', fontSize: '10px' }}>STOP LOSS</div>
                  <div style={{ fontWeight: 'bold', color: '#ef4444' }}>
                    {(selectedTrade.slPrice ?? selectedTrade.sl ?? selectedTrade.stopLoss) ? Number(selectedTrade.slPrice ?? selectedTrade.sl ?? selectedTrade.stopLoss).toFixed(5) : 'None'}
                  </div>
                </div>
                <div>
                  <div style={{ color: '#64748b', fontSize: '10px' }}>TAKE PROFIT</div>
                  <div style={{ fontWeight: 'bold', color: '#10b981' }}>
                    {(selectedTrade.tpPrice ?? selectedTrade.tp ?? selectedTrade.takeProfit) ? Number(selectedTrade.tpPrice ?? selectedTrade.tp ?? selectedTrade.takeProfit).toFixed(5) : 'None'}
                  </div>
                </div>
              </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginTop: '2px' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '2px', flex: 1 }}>
                  <label style={{ fontSize: '10px', color: '#94a3b8' }}>Volume (Lots):</label>
                  <input
                    type="number"
                    step="0.01"
                    min="0.01"
                    value={tradeVolumeInput}
                    onChange={(e) => setTradeVolumeInput(parseFloat(e.target.value) || 0.01)}
                    style={{
                      backgroundColor: '#1e293b',
                      border: '1px solid #334155',
                      borderRadius: '4px',
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '2px', flex: 1 }}>
                  <label style={{ fontSize: '10px', color: '#94a3b8' }}>Target Broker:</label>
                  <select
                    value={selectedOrderBroker}
                    onChange={(e) => setSelectedOrderBroker(e.target.value)}
                    style={{
                      backgroundColor: '#1e293b',
                      border: '1px solid #334155',
                      borderRadius: '4px',
              ...
              <div style={{ display: 'flex', gap: '6px', marginTop: '4px' }}>
                <button
                  ...
                  style={{
                    ...
                    fontSize: '11px',
                    fontWeight: 'bold',
                    cursor: executingOrder ? 'not-allowed' : 'pointer',
                    display: 'flex',
                    alignItems: 'center',
                    justifyContent: 'center',
                    gap: '6px',
                    boxShadow: '0 2px 8px rgba(37, 99, 235, 0.4)',
                    opacity: executingOrder ? 0.7 : 1,
                  }}
                >
                  {executingOrder ? <RefreshCw size={12} className="animate-spin" /> : <Play size={12} fill="#ffffff" />}
                  Run Trade Again
                </button>
                ...
                <button
                  ...
                  title="Center chart on trade entry"
                >
                  Jump To
                </button>
              </div>
              ...
              {tradeOrderResult && (
                <div style={{
                  ...
                  borderRadius: '4px',
                  fontSize: '11px',
                  backgroundColor: tradeOrderResult.status === 'success' ? 'rgba(16, 185, 129, 0.15)' : 'rgba(239, 68, 68, 0.15)',
                  color: tradeOrderResult.status === 'success' ? '#34d399' : '#f87171',
                  border: `1px solid ${tradeOrderResult.status === 'success' ? 'rgba(16, 185, 129, 0.3)' : 'rgba(239, 68, 68, 0.3)'}`,
                }}>
                  {tradeOrderResult.message}
                </div>
              )}
            </div>
          )}
