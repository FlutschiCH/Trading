...
export default function Dashboard() {
  ...
  const [newAccBroker, setNewAccBroker] = useState<'ctrader' | 'metatrader' | 'binance'>('ctrader');
  ...
  const [backtestTab, setBacktestTab] = useState<'trades' | 'equity' | 'weekly' | 'monthly' | 'hourly' | 'favourites' | 'comparison'>('trades');
  ...
  const [collapsedCards, setCollapsedCards] = useState<{ [key: string]: boolean }>(() => {
    try {
      const saved = localStorage.getItem('wyckoff_desk_collapsed_cards');
      return saved ? JSON.parse(saved) : {};
    } catch {
      return {};
    }
      ...
      const response = await fetch(`${API_BASE_URL}/api/backtest`, {
        ...
        body: JSON.stringify({
          ...
          strategy: rangeParams?.strategy || {
            ...
            entryCutoffTime,
            entryStabilityRule,
            indicatorRules: enabledIndicators
          },
          slVal: parseFloat(backtestSL) || 1.0,
          slType: backtestSLType,
          rr: parseFloat(backtestRR) || 2,
  ...
  return (
    <div style={styles.container}>
      ...
      {window.location.pathname === '/how-to' ? (
        ...
        <div style={{ padding: '24px', flex: 1, display: 'flex', flexDirection: 'column', height: 'calc(100vh - 80px)' }}>
          <NewsPanel />
        </div>
      ) : (
        <>
          ...
          <main style={styles.mainLayout}>
            ...
            {isMobile ? (
              <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '16px', paddingBottom: '70px' }}>
                {mobileTab === 'chart' ? (
                  <div style={{
                    width: '100%',
                    backgroundColor: 'var(--app-card-bg)',
              ...
              <div style={{
                ...
                {panelOrder.map((panelId) => {
                  ...
                  const headerStyle = {
                    ...
                    borderBottom: isCollapsed ? 'none' : '1px solid var(--app-card-border)',
                    fontSize: '12px',
                    fontWeight: 'bold',
                    color: 'var(--app-text)',
                  };
                  ...
                  const renderCollapseButton = (id: string) => (
                    <button
                      type="button"
                      className="no-drag"
                      onClick={(e) => {
                      ...
                      title={collapsedCards[id] ? 'Expand Card' : 'Collapse Card'}
                    >
                      <ChevronDown
                        size={15}
                        style={{
                          transform: collapsedCards[id] ? 'rotate(-90deg)' : 'rotate(0deg)',
                      ...
                      >
                        ...
                        >
                          <span style={{ display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
                            📊 Candlestick & Weis Wave Analysis Chart
                          </span>
                  ...
                  if (panelId === 'copytrader') {
                    return (
                      ...
                      >
                        ...
                        >
                          <span style={{ display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
                            🔄 Master / Slave Copytrader Manager
                          </span>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                            <span style={{ fontSize: '10px', color: '#9ca3af' }}>⋮ Drag Header</span>
