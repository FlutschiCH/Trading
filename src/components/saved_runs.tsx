...
export default function SavedRuns({ onClose, onLoadSavedBacktest, onAnalyzeBacktest }: SavedRunsProps) {
  ...
  const [sbSortField, setSbSortField] = useState<string>(() => {
    try {
      const saved = localStorage.getItem(STORAGE_KEY);
      if (saved) return JSON.parse(saved).sbSortField ?? 'created_at';
    } catch {}
    return 'created_at';
  });
  const [sbSortDir, setSbSortDir] = useState<'asc' | 'desc'>(() => {
    try {
      const saved = localStorage.getItem(STORAGE_KEY);
      if (saved) return JSON.parse(saved).sbSortDir ?? 'desc';
    } catch {}
    return 'desc';
  });
  const [sbFilterSymbol, setSbFilterSymbol] = useState<string>(() => {
    try {
      const saved = localStorage.getItem(STORAGE_KEY);
  ...
  const [sbFilterSession, setSbFilterSession] = useState<string>(() => {
    try {
      const saved = localStorage.getItem(STORAGE_KEY);
      if (saved) return JSON.parse(saved).sbFilterSession ?? 'all';
    } catch {}
    return 'all';
  });
  const [sbMaxDrawdown, setSbMaxDrawdown] = useState<string>(() => {
    try {
      const saved = localStorage.getItem(STORAGE_KEY);
      if (saved) return JSON.parse(saved).sbMaxDrawdown ?? '';
    } catch {}
    return '';
  });
  const [sbMinNetPnl, setSbMinNetPnl] = useState<string>(() => {
    try {
      const saved = localStorage.getItem(STORAGE_KEY);
      if (saved) return JSON.parse(saved).sbMinNetPnl ?? '';
    } catch {}
    return '';
  });
  const [sbMinWinRate, setSbMinWinRate] = useState<string>(() => {
    try {
      const saved = localStorage.getItem(STORAGE_KEY);
      if (saved) return JSON.parse(saved).sbMinWinRate ?? '';
    } catch {}
    return '';
  });
  const [sbMinTrades, setSbMinTrades] = useState<string>(() => {
    try {
      const saved = localStorage.getItem(STORAGE_KEY);
      if (saved) return JSON.parse(saved).sbMinTrades ?? '';
    } catch {}
    return '';
  });
  const [sbMinProfitFactor, setSbMinProfitFactor] = useState<string>(() => {
    try {
      const saved = localStorage.getItem(STORAGE_KEY);
      if (saved) return JSON.parse(saved).sbMinProfitFactor ?? '';
    } catch {}
    return '';
  });
  ...
  const filteredAndSortedList = React.useMemo(() => {
    return activeDisplayList
      .filter(item => {
        ...
        if (sbFilterSession !== 'all') {
          const activeSess = (item.sessions || []).filter((s: any) => s && (s.active === true || s.active === 1 || s.active === 'true'));
          const hasSessions = activeSess.length > 0;
          if (sbFilterSession === 'no_sessions' && hasSessions) return false;
          if (sbFilterSession === 'with_sessions' && !hasSessions) return false;
        }
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              <label style={{ color: 'var(--app-text, #cbd5e1)', fontSize: '12px', fontWeight: 500 }}>Sessions:</label>
              <select
                value={sbFilterSession}
                onChange={(e) => setSbFilterSession(e.target.value)}
                style={{
                  backgroundColor: 'var(--app-input-bg, #0f172a)',
                  color: 'var(--app-input-text, #f8fafc)',
                  border: '1px solid var(--app-input-border, #475569)',
              ...
              >
                ...
                <option value="no_sessions">No Sessions (24/7)</option>
                <option value="with_sessions">With Sessions Only</option>
              </select>
            </div>
